...
def complete(id):
    def _complete():
        ...
        c.execute("UPDATE todo SET completed = ? WHERE id = ?", (not todo[3], id)) #Cambio el estado del completed a True con la identacion del campo
        conn.commit()
        render_todo()
...
def render_todo():
    ...
    for i  in range(0, len(rows)):
        id = rows[i][0]
        completed = rows[i][3] #Iteramos i y vemos si esta completado o no
